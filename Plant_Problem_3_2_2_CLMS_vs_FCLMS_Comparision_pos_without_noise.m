...
mu_CLMS = 4e-2; % step-size of CLMS
mu_FCLMS = 2e-2;   % step-size of FCLMS
...
%% f1
        ...
        W_FCLMS1 = W_FCLMS1 +  mu_FCLMS*conj(e_FCLMS1)*U ...,
            +  (mu_FCLMS/gamma(2-f_FCLMS(1)))*conj(e_FCLMS1)*U.*(W_FCLMS1.^(1-f_FCLMS(1)));
        
%% f2
        y_FCLMS2 = (W_FCLMS2')*U;
        e_FCLMS2 = d(n) - y_FCLMS2;      

        W_FCLMS2 = W_FCLMS2 +  mu_FCLMS*conj(e_FCLMS2)*U ...,
            +  (mu_FCLMS/gamma(2-f_FCLMS(2)))*conj(e_FCLMS2)*U.*(W_FCLMS2.^(1-f_FCLMS(2)));
        
%% f3
        y_FCLMS3 = (W_FCLMS3')*U;
        e_FCLMS3 = d(n) - y_FCLMS3;      

        W_FCLMS3 = W_FCLMS3 +  mu_FCLMS*conj(e_FCLMS3)*U ...,
            +  (mu_FCLMS/gamma(2-f_FCLMS(3)))*conj(e_FCLMS3)*U.*(W_FCLMS3.^(1-f_FCLMS(3)));
        
   %% f4
        y_FCLMS4 = (W_FCLMS4')*U;
        e_FCLMS4 = d(n) - y_FCLMS4;      

        W_FCLMS4 = W_FCLMS4 +  mu_FCLMS*conj(e_FCLMS4)*U ...,
            +  (mu_FCLMS/gamma(2-f_FCLMS(4)))*conj(e_FCLMS4)*U.*(W_FCLMS4.^(1-f_FCLMS(4)));
        
   %% f5
        y_FCLMS5 = (W_FCLMS5')*U;
        e_FCLMS5 = d(n) - y_FCLMS5;      

        W_FCLMS5 = W_FCLMS5 +  mu_FCLMS*conj(e_FCLMS5)*U ...,
            +  (mu_FCLMS/gamma(2-f_FCLMS(5)))*conj(e_FCLMS5)*U.*(W_FCLMS5.^(1-f_FCLMS(5)));

   %% f6
        y_FCLMS6 = (W_FCLMS6')*U;
        e_FCLMS6 = d(n) - y_FCLMS6;      

        W_FCLMS6 = W_FCLMS6 +  mu_FCLMS*conj(e_FCLMS6)*U ...,
            +  (mu_FCLMS/gamma(2-f_FCLMS(6)))*conj(e_FCLMS6)*U.*(W_FCLMS6.^(1-f_FCLMS(6)));
        ...
        W_CLMS = W_CLMS +  mu_CLMS*conj(e_CLMS)*U;
...
save('results\Results_Problem3_2_2.mat')
