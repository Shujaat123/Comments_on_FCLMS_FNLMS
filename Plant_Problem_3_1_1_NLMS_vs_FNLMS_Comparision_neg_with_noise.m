...
mu_NLMS = 10e-1; % step-size of NLMS
mu_FNLMS = 5e-1; % step-size of FNLMS
f_FNLMS = [0.9 0.8 0.7 0.6 0.5 0.4]; % fractional powers

SNR = 10; % noise level in dB
h = [-10:10]'; % system impulse response
...
%% f1
        ...
        W_FNLMS1 = W_FNLMS1 +  mu_FNLMS*(e_FNLMS1)*U.*(1/(norm(U)^2+1e-10)) ...,
            +  (mu_FNLMS/gamma(2-f_FNLMS(1)))*(e_FNLMS1)*U.*(1/(norm(U)^2+1e-10)).*(W_FNLMS1.^(1-f_FNLMS(1)));
        
%% f2
        y_FNLMS2 = (W_FNLMS2')*U;
        e_FNLMS2 = d(n) - y_FNLMS2;      

        W_FNLMS2 = W_FNLMS2 +  mu_FNLMS*(e_FNLMS2)*U.*(1/(norm(U)^2+1e-10)) ...,
            +  (mu_FNLMS/gamma(2-f_FNLMS(2)))*(e_FNLMS2)*U.*(1/(norm(U)^2+1e-10)).*(W_FNLMS2.^(1-f_FNLMS(2)));
        
%% f3
        y_FNLMS3 = (W_FNLMS3')*U;
        e_FNLMS3 = d(n) - y_FNLMS3;      

        W_FNLMS3 = W_FNLMS3 +  mu_FNLMS*(e_FNLMS3)*U.*(1/(norm(U)^2+1e-10)) ...,
            +  (mu_FNLMS/gamma(2-f_FNLMS(3)))*(e_FNLMS3)*U.*(1/(norm(U)^2+1e-10)).*(W_FNLMS3.^(1-f_FNLMS(3)));
        
   %% f4
        y_FNLMS4 = (W_FNLMS4')*U;
        e_FNLMS4 = d(n) - y_FNLMS4;      

        W_FNLMS4 = W_FNLMS4 +  mu_FNLMS*(e_FNLMS4)*U.*(1/(norm(U)^2+1e-10)) ...,
            +  (mu_FNLMS/gamma(2-f_FNLMS(4)))*(e_FNLMS4)*U.*(1/(norm(U)^2+1e-10)).*(W_FNLMS4.^(1-f_FNLMS(4)));
        
   %% f5
        y_FNLMS5 = (W_FNLMS5')*U;
        e_FNLMS5 = d(n) - y_FNLMS5;      

        W_FNLMS5 = W_FNLMS5 +  mu_FNLMS*(e_FNLMS5)*U.*(1/(norm(U)^2+1e-10)) ...,
            +  (mu_FNLMS/gamma(2-f_FNLMS(5)))*(e_FNLMS5)*U.*(1/(norm(U)^2+1e-10)).*(W_FNLMS5.^(1-f_FNLMS(5)));

   %% f6
        y_FNLMS6 = (W_FNLMS6')*U;
        e_FNLMS6 = d(n) - y_FNLMS6;      

        W_FNLMS6 = W_FNLMS6 +  mu_FNLMS*(e_FNLMS6)*U.*(1/(norm(U)^2+1e-10)) ...,
            +  (mu_FNLMS/gamma(2-f_FNLMS(6)))*(e_FNLMS6)*U.*(1/(norm(U)^2+1e-10)).*(W_FNLMS6.^(1-f_FNLMS(6)));
   ...
   %% NLMS
        ...
        W_NLMS = W_NLMS +  mu_NLMS*(e_NLMS)*U.*(1/(norm(U)^2+1e-10));
...
title('Problem # 3.1.1')
save('results\Results_Problem3_1_1.mat')
